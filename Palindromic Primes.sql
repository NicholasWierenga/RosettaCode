...
DECLARE -- This filters through our table for palindromes.
    ...
    currrow INTEGER(6);
BEGIN
    SELECT COUNT(*) INTO rowsremain FROM primelist;
    SELECT seq1.CURRVAL INTO currrow FROM DUAL;
    currrow := currrow - rowsremain + 1;

WHILE x <= rowsremain LOOP
    
    SELECT prime INTO testpalprime FROM primelist WHERE tabrows = currrow;
    
        FOR c IN 1..CEIL(LENGTH(testpalprime)/2) LOOP
            IF SUBSTR(testpalprime, c, 1) != SUBSTR(testpalprime, -c, 1) THEN
                
                DELETE FROM primelist WHERE prime = testpalprime;
                x := x - 1;
...
SELECT PRIME AS "Palindromic Primes" FROM primelist;
